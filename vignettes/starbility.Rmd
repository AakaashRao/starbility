...
data("diamonds")
set.seed(42)
diamonds = diamonds %>% sample_frac(0.25)
```
...
```{r stability-1, fig.width=7.3, fig.height=4}
perm_controls = c(
  'Depth' = 'depth',
  'Width of top relative to widest point' = 'table'
)

create_stability_plot(data=diamonds, 
                      lhs='price', 
                      rhs='carat', 
                      perm=perm_controls)
```
...
```{r controls-1, fig.width=7.3, fig.height=4}
perm_controls = c(
  'Depth' = 'depth',
  'Width of top relative to widest point' = 'table',
...
```{r controls-2, fig.width=7.3, fig.height=4}
base_controls = c(
  'Diamond dimensions' = 'x + y + z'
)
perm_controls = c(
...
```{r fe-1, fig.width=7.3, fig.height=4}
perm_fe_controls = c(
  'Cut FE' = 'cut',
  'Color FE' = 'color',
  'Clarity FE' = 'clarity'
...
```{r fe-2, fig.width=7.3, fig.height=4}
...
Note that the first set of models in the plot are estimated without including any of the controls in `nonperm_fe`. This is `starbility`'s default behavior, but it can be disabled by setting `include_no_fe_specs=F`:

```{r fe-3, fig.width=7.3, fig.height=4}
create_stability_plot(data=diamonds, 
                      lhs='price', 
                      rhs='carat', 
                      perm=perm_controls, 
...
```{r clust-1, warning=F, fig.width=7.3, fig.height=4}
...
Let's now explore some options relating to plot aesthetics. The plots above have relatively few models,
so let's increase the number of models by specifying the dimensions as separate controls. Our model is estimated very precisely, so to make this example closer to a real-world application (in which coefficient estimates are often less precise), we'll add randomly-generated noise to the dependent variable.

```{r aesthetics-1, fig.width=7.3, fig.height=4}
diamonds$price = diamonds$price + rnorm(n=nrow(diamonds), sd=25*sd(diamonds$price))
...
```{r aesthetics-2, fig.width=7.3, fig.height=4}
create_stability_plot(data=diamonds, 
                      lhs='price', 
                      rhs='carat', 
                      perm=perm_controls, 
...
```{r aesthetics-3, fig.width=7.3, fig.height=4}
create_stability_plot(data=diamonds, 
                      lhs='price', 
                      rhs='carat', 
                      perm=perm_controls, 
...
```{r aesthetics-4, fig.width=7.3, fig.height=4}
create_stability_plot(data=diamonds, 
                      lhs='price', 
                      rhs='carat', 
...
```{r aesthetics-5, fig.width=7.3, fig.height=4}
create_stability_plot(data=diamonds, 
                      lhs='price', 
                      rhs='carat', 
                      perm=perm_controls, 
...
By default, `create_stability_plot` returns a grid (generated by `cowplot`). We can instead return the bottom and top ``ggplot2` panels individually using `combine=F`. The advantage is that we can use standard `ggplot2` syntax to adapt the panels. In the example below, I reverse the $y$-axis, remove the minor horizontal rules, and add an annotation. We can then combine the plots using the `combine_plots` helper function.

```{r split-1, fig.width=7.3, fig.height=4}
plots = create_stability_plot(data=diamonds, 
                              lhs='price', 
                              rhs='carat', 
                              perm=perm_controls, 
                              error_geom='ribbon',
